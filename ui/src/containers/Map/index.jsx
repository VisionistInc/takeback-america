...
class MapContainer extends React.Component {
  ...
  async componentDidMount() {
    try {
      const countiesPromise = fetch('/api/geojson/counties').then(res =>
        res.json()
      );

      const dropMarkersPromise = fetch('/api/geojson/drop_markers').then(res =>
        res.json()
      );
  ...
  render() {
    ...
    const { center, zoom, onCountyClick, takeBackFilter } = this.props;
